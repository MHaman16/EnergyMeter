...
bool ATM90E36::calibrationError()
{
  ...
  else 
  {
    ...
    Serial.println(CS3);
    /*unsigned short UBG = CommEnergyIC(READ, UgainB, 0xFFFF);
    Serial.println("UB_Gain" + UBG);*/
#endif
...
{  
  ...
  CommEnergyIC(WRITE, SagTh, 0x0000);       // Voltage sag threshold
  ...
  CommEnergyIC(WRITE, MMode0, 0x087);      // Mode Config (50 Hz, 3P4W)
  ...
  CommEnergyIC(WRITE, CSTwo, 0x0003);       // Checksum 2 
  
  //Set measurement calibration values (ADJUST)
  CommEnergyIC(WRITE, AdjStart, 0x5678);    // Measurement calibration
  CommEnergyIC(WRITE, UgainA, 0x3956);      // A SVoltage rms gain
  CommEnergyIC(WRITE, IgainA, 0x355F);      // A line current gain
  CommEnergyIC(WRITE, UoffsetA, 0x0000);    // A Voltage offset
  CommEnergyIC(WRITE, IoffsetA, 0x0000);    // A line current offset
  CommEnergyIC(WRITE, UgainB, 0x396F);      // B Voltage rms gain
  CommEnergyIC(WRITE, IgainB, 0x3566);      // B line current gain
  CommEnergyIC(WRITE, UoffsetB, 0x0000);    // B Voltage offset
  CommEnergyIC(WRITE, IoffsetB, 0x0000);    // B line current offset
  CommEnergyIC(WRITE, UgainC, 0x3967);      // C Voltage rms gain
  CommEnergyIC(WRITE, IgainC, 0x34B8);      // C line current gain
  CommEnergyIC(WRITE, UoffsetC, 0x0000);    // C Voltage offset
  CommEnergyIC(WRITE, IoffsetC, 0x0000);    // C line current offset
  CommEnergyIC(WRITE, IgainN, 0x0000);      // N line current gain
  CommEnergyIC(WRITE, IoffsetN, 0x0000);    // C line current offset
  CommEnergyIC(WRITE, CSThree, 0x52F2);     // Checksum 3
  delay(1000);
